...
[CreateAssetMenu(fileName = "Analyze", menuName = "CardEffects/Analyze")]
public class Analyze : CardEffect
{
        ...
        if (status != null)
        {
            if (statusEffect != null)
            {
                for (int i = 0; i < statusEffect.Length; i++)
                    targetEnemy.StatusGetter().AddStatus(statusEffect[i], enemy, -1, 3);
            }
        }
    }
}